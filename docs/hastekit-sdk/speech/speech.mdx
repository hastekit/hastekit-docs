...
| **Language** | `*string` | Optional. Language of the speech output. |
...
| **Speed** | `*int` | Optional. Integer speed multiplier for the generated audio (must be a whole number). When set, enables streaming mode (`IsStreamingRequest` returns true iff `Speed != nil`). |
...
| **RawFields** | `map[string]any` | Raw provider response fields. |
...
| **InputTokensDetails.CachedTokens** | `int` | Number of cached input tokens. |
| **OutputTokens** | `int` | Number of output tokens generated. |
| **OutputTokensDetails.ReasoningTokens** | `int` | Number of reasoning tokens. |
...
func main() {
    // Initialize the LLM client and bind a model
