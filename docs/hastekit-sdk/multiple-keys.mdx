...
When using the HasteKit SDK in direct mode, you can configure multiple keys by passing one or more `gateway.ProviderConfig` values to `hastekit.WithProviderConfigs`, assigning each `APIKeyConfig` a `Weight`.
...
func main() {
	...
	client, err := hastekit.NewWithOptions(
        hastekit.WithProviderConfigs(providerConfigs...),
    )
	if err != nil {
