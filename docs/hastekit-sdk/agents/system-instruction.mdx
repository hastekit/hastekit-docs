...
agent := client.NewAgent(&hastekit.AgentOptions{
    ...
    Instruction: client.RemotePrompt("my-prompt-name", "production-alias"),
    LLM:         model,
})
```
...
<Note>For this to work, you should have configured the `client` with the gateway's endpoint, organisation name, project name and an API key.</Note>
