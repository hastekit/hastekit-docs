...
- `ThreadID` - Optional. Thread to continue (or empty to start a new one)
...
- `StreamID` - Optional. Broker channel id; the server generates one if empty. The HTTP handler streams chunks back as Server-Sent Events as the run progresses.
