...
If you build the tool with the [`hastekit.NewTool`](/hastekit-sdk/agents/tools/function-tools) helper instead, mark it deferred with the `hastekit.WithDeferred` option:

```go
lookupTool := hastekit.NewTool(lookupUser,
	hastekit.WithName("lookup_user"),
	hastekit.WithDescription("Look up a user record by ID"),
	hastekit.WithDeferred(true), // keep out of the prompt until activated
)
```
...
Pass `"*"` to defer every tool the server exposes — the right default for a server with a large or changing catalog, since you no longer have to keep the list in sync with the server:
...
|---------------|---------|-------------|
| `query`       | string  | Required. Either `select:<tool_name>` to activate specific tools by name (comma-separate to activate several: `select:lookup_user,delete_user`), or plain keywords to search by. |
| `max_results` | integer | Optional. Maximum number of results a keyword search returns (default 5). Ignored by `select:`. |

A keyword query ranks deferred tools by how many of its terms appear in the tool's name or description — case-insensitive substring matching — and activates the best matches, up to `max_results`. A query that matches nothing answers `No matching deferred tools found for query: …` rather than silently activating nothing.
