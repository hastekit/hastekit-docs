...
`Execute()` (via the handle's `Result()`/`Wait()`) returns an `*agents.AgentOutput`. When approval is needed, its `Status` is `RunStatusPaused` and the pending tool calls are available in `PendingApprovals`:
...
type AgentOutput struct {
    ...
    PendingApprovals []responses.FunctionCallMessage `json:"pending_approvals"` // Tools awaiting approval
}
```

> Approval-required tools inside sub-agents (see [Agent as a tool](./agent-as-a-tool)) bubble up to the parent's `PendingApprovals`. The nested agent's pending approvals are propagated through `ToolCallResponse.PendingApprovals`, so the parent run pauses and surfaces them just like its own tools.

### Resuming Execution

To resume execution after user review, send a `FunctionCallApprovalResponseMessage` on the same thread. `Execute()` returns an `*AgentHandle`; the simplest way to wait for the resumed run is `handle.Result()`:
