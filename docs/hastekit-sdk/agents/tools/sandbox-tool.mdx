...
- **Tool registration** – `tools.NewBashTool(svc, image, env)` returns an `agents.Tool` that exposes **execute_bash_commands** to the LLM. Pass it in `AgentConfig.Tools` along with any other tools.
