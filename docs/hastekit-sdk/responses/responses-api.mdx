...
| **Include** | `[]Includable` | Additional data to include in the response (e.g., `IncludableReasoningEncryptedContent`).                    |
...
| **ExtraFields** | `map[string]any` | Optional. Provider-specific extra fields merged into the request payload. The reserved key `additional_headers` (a `map[string]any`) sets custom per-request HTTP headers.                        |

You can use `ExtraFields` to attach custom per-request HTTP headers via the reserved `additional_headers` key:
