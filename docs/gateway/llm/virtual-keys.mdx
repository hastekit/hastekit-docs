...
Virtual Keys are unique secret keys (prefixed with `sk-hk-`) that you can use instead of provider API keys in your applications. When a request comes in with a virtual key:
...
curl -X POST https://your-gateway.com/api/gateway/openai/v1/chat/completions \
  -H "Authorization: Bearer sk-hk-your-virtual-key-here" \
