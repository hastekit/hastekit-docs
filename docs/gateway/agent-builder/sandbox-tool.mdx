...
- **SANDBOX_DEFAULT_IMAGE** – Default Docker image for sandbox containers/pods (e.g. `praveenraj9495/hastekit-ai-sandbox:latest`). Must be an image that runs the HasteKit sandbox daemon (e.g. `hastekit-ai-sandbox sandbox-daemon`).
