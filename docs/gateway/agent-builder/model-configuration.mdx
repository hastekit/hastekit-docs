...
| **Anthropic** | Claude Opus, Sonnet, Haiku (including 1M-context variants) |
| **Gemini** | Gemini 2.5 Pro / Flash family |
...
Providers and their models are configured under **LLM Gateway → Providers**. Once a model is registered there, it shows up in the **Model ID** dropdown here.

## Required fields

- **Provider Type** — which provider account to call.
...
The **Agent Info** tab also exposes **Max Iteration** — the maximum number of agent loop turns (LLM call → tool calls → LLM call → ...) before the run is force-terminated. Leave empty to run without a limit. Use this as a safety net for agents that might otherwise loop indefinitely.
