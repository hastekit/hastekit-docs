...
6. Back in HasteKit, open **Agent Framework → [Credentials](/gateway/agent-builder/credentials)** and store the `client_id` and `client_secret` as a credential (or use an existing one with those keys).
...
- [Channels](/gateway/agent-builder/channels) — Channels deliver messages **to** the user (Slack, Telegram). Connectors let agents act **on behalf of** the user.
